...
public class ScoreProgressProc extends Thread {
	...
	private Map<String, JSONObject> progressMap;
	private String xxdm;
	private String progressMapKey;

	public ScoreProgressProc(String msg, String processId, int code, int ms, int eachPercent, int startPercent,
			Map<String, JSONObject> progressMap, String xxdm, String progressMapKey) {
	...
	private void updateProgress(String msg, int progress, String processId, int code,
			RedisOperationDAO redisOperationDAO, String progressMapKey) {
		JSONObject data = new JSONObject();
		JSONObject toFront = new JSONObject();
		Hashtable<String, JSONObject> progressMap = null;
		try {
			progressMap = (Hashtable<String, JSONObject>) redisOperationDAO.get(progressMapKey);
		} catch (Exception e1) {
			e1.printStackTrace();
		}
		...
		if (data.containsKey("progress") && data.getIntValue("progress") > progress) {
			...
			try {
				...
				redisOperationDAO.set(progressMapKey, progressMap, CacheExpireTime.temporaryDataMinExpireTime.getTimeValue());
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
