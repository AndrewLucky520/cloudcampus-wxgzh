...
//	//	SubCom[][] subComs = new SubCom[3][3];
//	//	for()
//		List<SubCom[][]> oneSubjectModes =  getInitOneSubjectMode(1,null);
//		System.out.println(oneSubjectModes.size());
//		for(SubCom[][] subComs: oneSubjectModes){
//			for(int i=0;i<3;i++){
//				System.out.println("");
//				for(int j=0;j<3;j++){
//					System.out.print("("+subComs[i][j].subjectIndex+","+subComs[i][j].seq+"),");
//				}
//			}
//			System.out.println("");
//			System.out.println("------------------------------------------");
...
//				{"政物化",0},
//				{"政物生",3}};
		PlacementTaskConfig config = new PlacementTaskConfig(50,55,optSubjectLessonsMap,proSubjectLessonsMap,SelectType.ThreeFromSeven);
	...
	public static void test(Map<String,Integer> subjectCombinationsMap,PlacementTaskConfig config){
	    ...
	    allConflicts.get(0).get(1).seqs[0]= new ArrayList(Arrays.asList(new Integer[]{1,4,3}));
	    compute7X3(subjectCombinations,allConflicts.get(0),computeMap,config);
		...
		for(Conflict conflict:allConflicts.get(0)){
			System.out.println("-------------------");
	...
	public static  List<ConflictComputeResult> compute(Map<Integer,Subject>[] subjectIdMaps,List<SubjectCombination> subjectCombinations,PlacementTaskConfig config){
		...
		for(List<Conflict> conflicts : allConflicts){
			...
			do{
				...
				boolean isOk = ConflictComputer.compute7X3(subjectCombinations,conflicts,result,config);
				if(isOk){
	 ...
	 */
	private static boolean compute(List<SubjectCombination> subjectCombinations,
		...
		for(SubjectCombination subjectCombination:subjectCombinations){
			...
			subjectCombination.setOrderOptSubjectIds(new ArrayList(subjectCombination.subjectIds[CD_OPT]));
			Stack<SubSubjectCombination[][]> resultStack = new Stack();
			for(List<Integer> orderProSubjects : orderProSubjectsList){
				...
				System.out.println("subjectCombination:"+subjectCombination.getSubjectIdsStr()
						+"   orderProSubjectIds:"+JSONObject.toJSONString(subjectCombination.getOrderProSubjectIds())
						+"	orderOptSubjectIds"+JSONObject.toJSONString(subjectCombination.getOrderOptSubjectIds()));
				System.out.println("orderProSubjectIds:"+JSONObject.toJSONString(subjectCombination.getOrderProSubjectIds()));
				System.out.println("orderOptSubjectIds:"+JSONObject.toJSONString(subjectCombination.getOrderOptSubjectIds()));
...
//								System.out.println("-----------------------------------------");
//								for(int i=0;i<3;i++){
//									System.out.println("("+subComs[i][0].subjectIndex+","+subComs[i][0].seq+"),"
//											+"("+subComs[i][1].subjectIndex+","+subComs[i][1].seq+"),"
//											+"("+subComs[i][2].subjectIndex+","+subComs[i][2].seq+")");
//								}
					//		System.out.println("-----------------------------------------");
							if(checkConflict(subComs,subjectCombination,conflicts,config.seqCounts[CD_OPT])){
								isOk = true;
								resultStack.pop();
								resultStack.push(subComs);
								//检查最后一个科目是否有子志愿拍到1，2，3序列
								boolean canArrange = false;
								for(SubSubjectCombination subCom : subComs[0]){
			...
			for(List<Integer> orderProSubjects : orderProSubjectsList){
				subjectCombination.setOrderProSubjectIds(orderProSubjects);
				List<ConflictType> conflictTypes = Arrays.asList(new ConflictType[]{ConflictType.TYPE_1,ConflictType.TYPE_2,ConflictType.TYPE_3,ConflictType.TYPE_4,ConflictType.TYPE_5,ConflictType.TYPE_6});
				for(int i=1;i<conflictTypes.size();i++){
					subjectCombination.setConfictType(conflictTypes.get(i));
				//	System.out.println("-----------------------type:"+subjectCombination.getConfictType());
...
//								System.out.println("-----------------------------------------");
//								for(int k=0;k<3;k++){
//									System.out.println("("+subComs[k][0].subjectIndex+","+subComs[k][0].seq+"),"
//											+"("+subComs[k][1].subjectIndex+","+subComs[k][1].seq+"),"
//											+"("+subComs[k][2].subjectIndex+","+subComs[k][2].seq+")");
//								}
//								System.out.println("-----------------------------------------");
								if(checkConflict(subComs,subjectCombination,conflicts,config.seqCounts[CD_OPT])){
									isOk = true;
...
//		}
	 ...
	 * @return Map<SubjectCombination,SubSubjectCombination[seqIndex][subjectIndex]>
	 */
	public static boolean compute7X3(List<SubjectCombination> subjectCombinations,
			List<Conflict> conflicts,
			Map<SubjectCombination,
			SubSubjectCombination[][]> result,
			PlacementTaskConfig config){
...
//						System.out.println("---------------------"+JSONObject.toJSONString(subjectCombination.orderOptSubjectIds)+"----"+JSONObject.toJSONString(subjectCombination.orderProSubjectIds));
//						
		    ...
		    for(int i=1;i<conflictTypes.size();i++){
				...
				for(List<Integer> orderOptSubjects : orderOptSubjectsList){
			    	...
			    	for(List<Integer> orderProSubjects : orderProSubjectsList){
						...
						for(SubSubjectCombination[][] subComs:subComsList){
//							System.out.println("-------------------"+subjectCombination.getSubjectIdsStr()+"----type:"+subjectCombination.getConfictType()+"--"+JSONObject.toJSONString(subjectCombination.orderOptSubjectIds)+"----"+JSONObject.toJSONString(subjectCombination.orderProSubjectIds));
