 ...
 * @version:1.0
 * @Description: excel实体封装导入导出工具 
 * @author 武洋 ---智慧校
 * @date 2015年3月3日
 */
/*
 * 具体实现可参考com.talkweb.common.tools.ExcelToolDemo包内的实现
 * ExcelUtil工具类实现功能:
...
public class ExcelUtil<T> {
    ...
    public List<T> importExcel(String sheetName, InputStream input,String fileType) {
                ...
                for (int i = 1; i < rows; i++) {// 从第2行开始取数据,默认第一行是表头.
                    ...
                    for (int j = 0; j < cellNum; j++) {
						...
						case HSSFCell.CELL_TYPE_NUMERIC:
							...
							cvalue = (String)df.format(cell.getNumericCellValue());
							break;
                        ...
                        if(null == field) throw new IllegalArgumentException("找不到对应字段行["+(i+1)+"]列["+(j+1)+"]");
                        
                        // 取得类型,并根据对象类型设置值.
                        Class<?> fieldType = field.getType();
                        if (String.class == fieldType) {
                            field.set(entity, String.valueOf(cvalue.trim()));
                        } else if ((Integer.TYPE == fieldType) || (Integer.class == fieldType)) {
                            field.set(entity, Integer.parseInt(cvalue));
                        } else if ((Long.TYPE == fieldType) || (Long.class == fieldType)) {
                            field.set(entity, Long.valueOf(cvalue));
                        } else if ((Float.TYPE == fieldType) || (Float.class == fieldType)) {
                            field.set(entity, Float.valueOf(cvalue));
                        } else if ((Short.TYPE == fieldType) || (Short.class == fieldType)) {
                            field.set(entity, Short.valueOf(cvalue));
                        } else if ((Double.TYPE == fieldType) || (Double.class == fieldType)) {
                            field.set(entity, Double.valueOf(cvalue));
                        } else if (Character.TYPE == fieldType) {
                            if ((cvalue != null) && (cvalue.length() > 0)) {
                                field.set(entity, Character.valueOf(cvalue.charAt(0)));
                            }
                        }
